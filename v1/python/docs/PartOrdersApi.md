...
# **get_part_orders**
> PartOrdersResponse get_part_orders(limit, offset, status=status, part_order_number=part_order_number, sort=sort)
...
sort = ['sort_example'] # list[str] | Field(s) to sort by. Ascending order by default, use `sort=field,desc` to specify descending order. Sortable fields are: `part_order_number`, `due_date`, `flushed_at` (optional)
    ...
    try:
        # Fetch part orders
        api_response = api_instance.get_part_orders(limit, offset, status=status, part_order_number=part_order_number, sort=sort)
        pprint(api_response)
    except ApiException as e:
